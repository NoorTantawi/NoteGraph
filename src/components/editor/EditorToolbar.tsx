...
export function EditorToolbar({ view }: EditorToolbarProps) {
  ...
  return (
    <div className="flex items-center gap-1 p-2 border-b border-[var(--border)] bg-[var(--bg-secondary)] flex-shrink-0">
      <IconButton icon={Bold} title="Bold (Cmd+B)" onClick={(v: EditorView) => wrapSelection(v, '**', '**')} />
      <IconButton icon={Italic} title="Italic (Cmd+I)" onClick={(v: EditorView) => wrapSelection(v, '*', '*')} />
      <IconButton icon={Strikethrough} title="Strikethrough (Cmd+Shift+X)" onClick={(v: EditorView) => wrapSelection(v, '~~', '~~')} />
      ...
      <IconButton icon={Code} title="Code (Cmd+E)" onClick={(v: EditorView) => wrapSelection(v, '`', '`')} />
      <div className="w-px h-4 bg-[var(--border)] mx-1" />
